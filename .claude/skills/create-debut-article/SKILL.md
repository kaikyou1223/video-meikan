...
| デビュー作品CID | 作品カード | **必ずDB照会で取得**（下記SQL）。CIDを推測・捏造しないこと |
...
#### 警告ボックス
CSSが⚠️アイコンをタイトル前に自動付与するため、タイトルに⚠️を含めないこと。
```
:::alert[タイトル]
警告テキスト
:::
...
- [ ] FANZA作品リンクのCIDが正しいか
- [ ] **CID-女優整合性**: 各女優セクションのCID（画像URL・FANZAリンク・サンプル）がその女優の作品か（`validate_articles.php` で自動検証される）
